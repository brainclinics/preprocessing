...
from pio import FilepathFinder
from interprocessing import interdataset as intds
...
def end2end_alphaPowerandiAPF(varargs):
    ...
    exclude = ['BAD', '._' ,'Apple']
    files = FilepathFinder('.npy', varargs['preprocpath'], exclude =exclude)
    files.get_filenames()

    tsvdat = pd.read_csv(varargs['sourcepath']+'/participants.csv',sep = ',')
